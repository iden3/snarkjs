var snarkjs = (function(exports, _iden3_binfileutils, ffjavascript, r1csfile) {
	...
	*/
	...
	async function builder(code, options) {
		...
		if (code instanceof WebAssembly.Instance) {
			...
			if (options.memorySize) {
				...
				if (memorySize < 0) throw new Error("Invalid memory size");
			}
			let memoryAllocated = false;
			while (!memoryAllocated) try {
				memory = new WebAssembly.Memory({ initial: memorySize });
				memoryAllocated = true;
			} catch (err) {
				if (memorySize <= 1) throw err;
				...
				runtime: {
					...
					showSharedRWMemory: function() {
						...
						if (majorVersion >= 2 && (minorVersion >= 1 || patchVersion >= 7)) {
							if (msgStr !== "") msgStr += " ";
							const msg = ffjavascript.Scalar.fromArray(arr, 4294967296).toString();
							msgStr += msg;
						} else console.log(ffjavascript.Scalar.fromArray(arr, 4294967296));
					},
					error: function(code, pstr, a, b, c, d) {
						let errStr;
						if (code === 7) errStr = p2str(pstr) + " " + wc.getFr(b).toString() + " != " + wc.getFr(c).toString() + " " + p2str(d);
						else if (code === 9) errStr = p2str(pstr) + " " + wc.getFr(b).toString() + " " + p2str(c);
						else if (code === 5 && options.sym) errStr = p2str(pstr) + " " + options.sym.labelIdx2Name[c];
						else errStr = p2str(pstr) + " " + a + " " + b + " " + c + " " + d;
					...
					logGetSignal: function(signal, pVal) {
						if (options.logGetSignal) options.logGetSignal(signal, wc.getFr(pVal));
					},
					logSetSignal: function(signal, pVal) {
						if (options.logSetSignal) options.logSetSignal(signal, wc.getFr(pVal));
					},
					logStartComponent: function(cIdx) {
						if (options.logStartComponent) options.logStartComponent(cIdx);
					},
		...
		if (typeof instance.exports.getVersion == "function") majorVersion = instance.exports.getVersion();
		if (typeof instance.exports.getMinorVersion == "function") minorVersion = instance.exports.getMinorVersion();
		if (typeof instance.exports.getPatchVersion == "function") patchVersion = instance.exports.getPatchVersion();
		...
		if (majorVersion === 2) wc = new WitnessCalculatorCircom2(instance, sanityCheck);
		else if (majorVersion === 1) {
			if (codeIsWebAssemblyInstance) throw new Error("Loading code from WebAssembly instance is not supported for circom version 1");
			wc = new WitnessCalculatorCircom1(memory, instance, sanityCheck);
		} else throw new Error(`Unsupported circom version: ${majorVersion}`);
		return wc;
		function getMessage() {
			let message = "";
	...
	var WitnessCalculatorCircom1 = class {
		constructor(memory, instance, sanityCheck) {
			...
			for (let i = 0; i < this.n32; i++) arr[this.n32 - 1 - i] = this.i32[(pRawPrime >> 2) + i];
			this.prime = ffjavascript.Scalar.fromArray(arr, 4294967296);
		...
		getFr(p) {
			const self = this;
			const idx = p >> 2;
			if (self.i32[idx + 1] & 2147483648) {
				const arr = new Array(self.n32);
				for (let i = 0; i < self.n32; i++) arr[self.n32 - 1 - i] = self.i32[idx + 2 + i];
				const res = self.Fr.e(ffjavascript.Scalar.fromArray(arr, 4294967296));
				if (self.i32[idx + 1] & 1073741824) return fromMontgomery(res);
				else return res;
			} else if (self.i32[idx] & 2147483648) return self.Fr.e(self.i32[idx] - 4294967296);
			else return self.Fr.e(self.i32[idx]);
		...
		setFr(p, v) {
			...
			if (self.Fr.geq(v, minShort) && self.Fr.leq(v, maxShort)) {
				let a;
				if (self.Fr.geq(v, self.Fr.zero)) a = ffjavascript.Scalar.toNumber(v);
				else {
			...
			self.i32[p >> 2] = 0;
			self.i32[(p >> 2) + 1] = 2147483648;
			const arr = ffjavascript.Scalar.toArray(v, 4294967296);
			for (let i = 0; i < self.n32; i++) {
				const idx = arr.length - 1 - i;
				if (idx >= 0) self.i32[(p >> 2) + 2 + i] = arr[idx];
				else self.i32[(p >> 2) + 2 + i] = 0;
			}
		}
	};
	var WitnessCalculatorCircom2 = class {
		...
		async calculateWitness(input, sanityCheck) {
			...
			for (let i = 0; i < this.witnessSize; i++) {
				...
				w.push(ffjavascript.Scalar.fromArray(arr, 4294967296));
			}
			return w;
		}
	...
	*/
	...
	var Keccak256Transcript = class {
		...
		getChallenge() {
			...
			const value = ffjavascript.Scalar.fromRprBE(keccak_256(buffer));
	...
	return exports;
})({}, _iden3_binfileutils, ffjavascript, r1csfile);
