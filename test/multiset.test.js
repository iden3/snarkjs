...
describe("snarkjs: Multiset tests", function () {
    this.timeout(150000);
    ...
    it("should return a sorted version of two multisets", async () => {
        ...
        let length1 = getRandomValue(1024);
        let length2 = getRandomValue(1024);

        let buff1 = new BigBuffer(length1 * Fr.n8);
        let num1 = getRandomValue(8);
        for (let i = 0; i < length1; i++) {
            buff1.set(Fr.e((i + 1) * num1), i * curve.Fr.n8);
        }

        let buff2 = new BigBuffer(length2 * Fr.n8);
        let num2 = getRandomValue(8);
        for (let i = 0; i < length2; i++) {
            buff2.set(Fr.e((i + 1) * num2), i * curve.Fr.n8);
        }

        let multiset1 = new Multiset(0, Fr);
        multiset1.buff = buff1;
        let multiset2 = new Multiset(0, Fr);
        multiset2.buff = buff2;

        let sortedMultiset = multiset1.sortedBy(multiset2);

        for (let i = 1; i < sortedMultiset.length(); i++) {
            let diff = curve.Fr.sub(
                sortedMultiset.buff.slice((i - 1) * curve.Fr.n8, i * curve.Fr.n8),
                sortedMultiset.buff.slice(i * curve.Fr.n8, (i + 1) * curve.Fr.n8));
