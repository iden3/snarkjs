...
class RangeCheckProver {
    async computeProof(preInput, witnesses, curve, logger, PTau) {
        ...
        async function round1() {
            ...
            for (let i = 0; i < N; i++) {
                if (logger && i % 10000 === 0) {
                    logger.debug("Creating preprocessed polynomials for range check");
                }
            ...
            evaluations.lt = await Evaluations.fromPolynomial(polynomials.LT, Fr, logger);
            ...
            if (polynomials.LT.degree() >= DOMAIN_SIZE) {
                throw new Error("range_check: LT Polynomial is not well calculated");
            }
            ...
            proof.addPolynomial("LT", await multiExpPolynomial("LT", polynomials.LT));
        ...
        async function round3() {
            ...
            for (let i = 0; i < DOMAIN_SIZE * 4; i++) {
                ...
                const i_n8 = i * Fr.n8;
                const i_wn8 = ((i + DOMAIN_SIZE * 4 + 4) % (DOMAIN_SIZE * 4)) * Fr.n8;
                ...
                const z_wi = evaluations.Z.eval.slice(i_wn8, i_wn8 + Fr.n8);
                const f_i = evaluations.F.eval.slice(i_n8, i_n8 + Fr.n8);
                const lt_i = evaluations.lt.eval.slice(i_n8, i_n8 + Fr.n8);
                const h1_i = evaluations.H1.eval.slice(i_n8, i_n8 + Fr.n8);
                const h1_wi = evaluations.H1.eval.slice(i_wn8, i_wn8 + Fr.n8);
                ...
                // IDENTITY A) Z(x)(γ + f(x))(γ + lt(x)) = Z(xω)(γ + h1(x))(γ + h2(x))
                const a0f = Fr.add(challenges.gamma, f_i);
                const a0t = Fr.add(challenges.gamma, lt_i);
                const [a0, a0p] = mul3(z_i, a0f, a0t, zp_i, fp_i, tp_i, i % 4, Fr);

                const a1h1 = Fr.add(challenges.gamma, h1_i);
                const a1h2 = Fr.add(challenges.gamma, h2_i);
                const [a1, a1p] = mul3(z_wi, a1h1, a1h2, zWp_i, h1p_i, h2p_i, i % 4, Fr);
            ...
            for (let i = 0; i < polynomials.splitQ.length; i++) {
        ...
        async function round4() {
            ...
            proof.addEvaluation("lt", polynomials.LT.evaluate(challenges.xi));
        ...
        async function round5() {
            ...
            const a0t = Fr.add(challenges.gamma, proof.evaluations.lt);
            ...
            for (let i = 1; i < polynomials.splitQ.length; i++) {
                polynomials.splitQ[i].mulScalar(xinTotal);
            ...
            polynomials.Wxi.add(polynomials.LT, challenges.v[2]);
            ...
            polynomials.Wxi.subScalar(Fr.mul(challenges.v[2], proof.evaluations.lt));
