...
export default async function plonkSetup(r1csName, ptauName, zkeyName, logger) {
    ...
    if (r1cs.useCustomGates) {
        nSections += customGates.gates.length * 2;
    }
        ...
        if (r1cs.useCustomGates) {
            for (let i = 0; i < r1cs.customGatesUses.length; i++) {
                ...
                constraints.forEach(ctr => {
                    let qArr = Array(r1cs.customGates.length);
                    for (let i = 0; i < r1cs.customGates.length; i++) {
                        qArr[i] = r1cs.customGatesUses[i].id === i ? ctr.qk : curve.Fr.zero;
                    }
                    plonkConstraints.push([ctr.sl, ctr.sr, ctr.so, ctr.qm, ctr.ql, ctr.qr, ctr.qo, ctr.qc, ...qArr]);
                });
            }
        }
    }
    ...
    async function writeZKeyCustomGateQMap(idx) {
        ...
        const sectionNum = customGates.gates[idx].qMapSectionId;
        await startWriteSection(fdZKey, sectionNum);
        await writeP4(Q);
        await endWriteSection(fdZKey);
    ...
    async function writeHeaders() {
        ...
        if (r1cs.useCustomGates) {
            for (let i = 0; i < customGates.gates.length; i++) {
                const preInput = customGates.gates[i].preprocessedInput();
                vk.customGates[i].preInput = {};
                vk.customGates[i].preInput.data = preInput.data;

                const keys = customGates.gates[i].preprocessedInputKeys;

                if (logger) logger.debug(`writing custom gate ${customGates.gates[i].name} header section`);

                await startWriteSection(fdZKey, customGates.gates[i].preprocessedSectionId);
