...
export default async function plonkExportSolidityCallData(_proof, _pub) {
    ...
    const useCustomGates = "customGates" in _proof && Array.isArray(_proof["customGates"]) && _proof["customGates"].length > 0;
    ...
    if (useCustomGates) {
        let offset = G1.F.n8 * (2 * 9) + Fr.n8 * 7;
        for (let i = 0; i < _proof.customGates.length; i++) {
            let cg = FactoryCG.create(_proof.customGates[i].id, {parameters: {}});
            let cgProof = unstringifyBigInts(_proof.customGates[i].proof);

            let keys = cg.solidityCallDataKeys();

            if(keys.polynomials) {
                for (let j = 0; j < keys.polynomials.length; j++) {
                    G1.toRprUncompressed(proofBuff, offset, G1.e(cgProof.polynomials[keys.polynomials[j]]));
                    offset += G1.F.n8 * 2;
                }
            }
            if(keys.evaluations) {
                for (let j = 0; j < keys.evaluations.length; j++) {
                    Fr.toRprBE(proofBuff, offset, Fr.e(cgProof.evaluations[keys.evaluations[j]]));
                    offset += Fr.n8;
                }
            }
        }
